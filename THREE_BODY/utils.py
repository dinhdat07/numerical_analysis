...
def compute_energy_3D(pos, vel):
    ...
    # Động năng
    T = 0.5 * m1 * (vx1**2 + vy1**2 + vz1**2) + \
        0.5 * m2 * (vx2**2 + vy2**2 + vz2**2) + \
        0.5 * m3 * (vx3**2 + vy3**2 + vz3**2)
...
def rk45_step_3D(state, dt):
    k1 = derivatives_3D(state)
    k2 = derivatives_3D(state + dt * (1/4) * k1)
    k3 = derivatives_3D(state + dt * ((3/32)*k1 + (9/32)*k2))
    k4 = derivatives_3D(state + dt * ((1932/2197)*k1 - (7200/2197)*k2 + (7296/2197)*k3))
    k5 = derivatives_3D(state + dt * ((439/216)*k1 - 8*k2 + (3680/513)*k3 - (845/4104)*k4))
    k6 = derivatives_3D(state + dt * ((-8/27)*k1 + 2*k2 - (3544/2565)*k3 + (1859/4104)*k4 - (11/40)*k5))
...
def rk54_step_3D(state, dt):
    k1 = derivatives_3D(state)
    k2 = derivatives_3D(state + dt * (1/5) * k1)
    k3 = derivatives_3D(state + dt * ((3/40)*k1 + (9/40)*k2))
    k4 = derivatives_3D(state + dt * ((44/45)*k1 - (56/15)*k2 + (32/9)*k3))
    k5 = derivatives_3D(state + dt * ((19372/6561)*k1 - (25360/2187)*k2 + (64448/6561)*k3 - (212/729)*k4))
    k6 = derivatives_3D(state + dt * ((9017/3168)*k1 - (355/33)*k2 + (46732/5247)*k3 + (49/176)*k4 - (5103/18656)*k5))
    k7 = derivatives_3D(state + dt * ((35/384)*k1 + (500/1113)*k3 + (125/192)*k4 - (2187/6784)*k5 + (11/84)*k6))
